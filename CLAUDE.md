...
- **This is a production-ready library suite handling traffic at billion-dollar scale.** Every feature must be implemented with production scenarios in mind. No half-measures, no "future improvements" hidden behind TODOs, no shortcuts that leak complexity onto the user.
...
- **NEVER document broken behaviour as "GAP" or "known limitation" instead of fixing it.** When a test surfaces a real bug in production code, the test is doing its job — its failure is the signal. The correct response is ALWAYS to fix the production code so the test passes against the corrected behaviour, not to soften the assertion (`t.Logf`, GAP-pin, `assert.True(false-state)`) so the test rides the broken state. Documenting bugs instead of fixing them produces a documented broken product — the worst possible outcome at this scale. The only acceptable form of "soften and document" is when the underlying issue is a deliberate design tradeoff that has been explicitly weighed; in that case the rationale, with concrete numbers, lives in source-level godoc/TSDoc, not in test-suppression boilerplate.
- **Security posture is non-negotiable.** When weighing a tradeoff between simpler-but-less-secure and harder-but-fail-closed, choose fail-closed every time. Spoofing defences, CRLF/NUL sanitisation, header strip on auth-validated routes, fail-closed FailPolicy under store outage, etc. — these are not optional polish.
- **Operator-only surfaces (probes, admin, metrics, debug) live on a separate listener.** Health probes, future pprof, metrics scrape, and admin endpoints MUST NOT share a socket with public client traffic. Mixing them blurs the trust boundary and forces every endpoint to mount its own auth middleware. Stage 1 of the gateway-server uses a single port for `/healthz` and `/readyz` as a transitional state with a roadmap entry to split; do not extend the convention by adding more operator endpoints to the public listener.
- **Three-tier test coverage is mandatory for gateway changes.** Every behaviour shipped on `apps/gateway-server` requires (a) unit tests that pin the function-level contract, (b) integration tests against real NATS (testcontainers) for any code that touches the wire, (c) e2e tests against the live three-process stack for any user-facing wire contract. "Unit-only" is acceptable only for pure helpers with no I/O — never for handlers, middleware, store backends, lifecycle code. testcontainers can simulate NATS-kill (`container.Stop`/`Start`) and JetStream-only outage (delete the stream backing the bucket) — both are in scope.
- **Production code under test must survive its own resilience tests.** When a test simulates a real outage scenario (NATS dies, JetStream deleted, request mid-flight during shutdown) and the production code fails in a non-graceful way (silent goroutine death, no operator signal, K8s rotation amplification), that is a bug to fix at the source — not a "harness limitation" to skip past.
