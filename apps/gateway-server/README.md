...
| `RATELIMIT_MEMORY_MAX_ENTRIES` | int64 | `1000000` | Maximum live-key cardinality the in-process `MemoryStore` admits before refusing new keys with `ErrMemoryStoreSaturated` (existing keys keep resolving). Defends against carpet-bombing attacks that rotate the source IP every request — without a cap the store would hold them all in RAM until the TTL sweeper caught up. Zero disables the cap. At roughly 128 bytes per entry, the default fits in about 122 MiB (1 000 000 × 128 B ≈ 128 MB decimal / 122 MiB binary). See `Config.RateLimitMemoryMaxEntries`. |
