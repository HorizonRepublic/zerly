...
const (
	...
	// zeroTimeUnixNano is the UnixNano value of time.Time{} (the zero time,
	// year 0001). We special-case this value in decodeTAT because time.Unix()
	// has a bug reconstructing very old dates from their UnixNano values.
	zeroTimeUnixNano int64 = -6795364578871345152
)
...
func decodeTAT(b []byte) (time.Time, error) {
	...
	// Special-case the zero time (time.Time{}).
	// time.Unix() has a bug reconstructing very old dates from their UnixNano values;
	// we detect and reconstruct the zero time explicitly.
	if ns == zeroTimeUnixNano {
